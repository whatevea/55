      ...
      <div className="bg-green-50 shadow-md rounded-md overflow-hidden mt-4">
        <div>
                ...
                <div>
                  {jobData?.budgetType === "hourly" ? (
                    <div>
                      ...
                      :{" "}
                      <span className="capitalize inline-block font-bold mx-1 text-sm">
                        ${jobData?.budgetHourlyMin}
                      </span>
                      -
                      <span className="capitalize inline-block font-bold mx-1 text-sm">
                        ${jobData?.budgetHourlyMax}
                      </span>
                      <span className="capitalize inline-block font-bold mx-1 text-sm">
                        Duration: {jobData?.scopeDuration}
                      </span>
                      <span className="capitalize inline-block font-bold mx-1 text-sm">
                        Experience: {jobData?.scopeExperience}
                      </span>
                    </div>
                  ) : (
                    <div>
          ...
          <div className="p-4">
            ...
            <p className="text-sm text-black mt-2">{jobData?.description}</p>
          </div>
          <div className="p-4 border-t border-gray-200">
            ...
            <ul className="flex flex-wrap mt-2">
              {jobData?.skills_required.map((item) => (
                <li
                  className="m-1 px-2 py-1 rounded-full bg-green-200 text-black text-sm font-semibold"
                  key={item}
                >
                  {item}
                </li>
        ...
        <div className="p-4 flex flex-col w-3/4 gap-4 mx-auto">
          <h1 className="text-3xl font-bold mt-4 text-green-600 mb-4 mx-auto">
            Jop Applicants
          </h1>
          {applierData &&
            applierData.map((applier, index) => {
              ...
              return applier.cover_letter ||
                applier.offered_amount ||
                applier.attachment_url ? (
                <Accordion
                  key={applier._id}
                  ...
                  <div className="">
                    <div className="bg-gray-200 border border-gray-300 shadow-lg rounded-md p-6 w-full mt-8">
                      <h2 className="text-2xl font-semibold text-green-700 mb-4">
                        Applicant: {user?.fname?.toUpperCase()}{" "}
                        ...
                        <div>
                          ...
                          {applier.attachment_urls.length > 0 ? (
                            applier.attachment_urls.map((attachment, index) => {
                              ...
                              const filename =
                                parts?.length === 2 ? parts[1] : attachment;
                              return (
                      ...
                      <div className="mt-4">
                        <button
                          className="rounded-md text-white bg-green-600 hover:bg-green-500 px-3 py-1.5 text-base font-semibold"
