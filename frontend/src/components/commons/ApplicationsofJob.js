        ...
        async function fetchData() {
            try {
                ...
                if (req.data && req.data.data) {
                    ...
                    // Process the second response as needed
                }
            } catch (error) {
                ...
                <div className="p-4 flex items-center justify-between">
                    {
                        applierData && applierData.length > 0 ? (
                            applierData.map((applier)=>(
                                <div className="bg-gray-200 border border-gray-300 shadow-lg rounded-md p-6 max-w-xl mx-auto mt-8">
                                <h2 className="text-2xl font-semibold text-green-700 mb-4">Applicant Information</h2>
                                <div className="flex flex-col space-y-4">
                                    <div>
                                        <label className="text-gray-600">Attachment Url:</label>
                                        <p className="text-green-700">{applierData.attachment_url}</p>
                                    </div>
                                    <div>
                                        <label className="text-gray-600">Cover Letter:</label>
                                        <p className="text-green-700">{applierData.cover_letter}</p>
                                    </div>
                                    <div>
                                        <label className="text-gray-600">Offered Amount:</label>
                                        <p className="text-green-700">${applierData.offered_amount}</p>
                                    </div>
                                </div>
                            </div>
                            ))                            
