...
const ChatArea = ({ contractId }) => {
  ...
  useEffect(() => {
    const getContract = async () => {
      ...
      try {
        ...
        const contract = response.data;
        let receiverId =
          userData.userData.user_type === "hire"
            ? contract[0].employee?._id
            : contract[0].hirer?._id;
        setReceiverUser(receiverId);
      } catch (error) {
        console.error("Error fetching contract:", error);
      }
  ...
  useEffect(() => {
    ...
    newSocket.on("connect", () =>
      console.log(
        "Connected to socket server and the newSocket.id is",
        newSocket.id
      )
    );
    newSocket.emit("joinRoom", contractId);
    ...
    newSocket.on("message-received", (message) => {
      ...
      setConversationMessages((prevConversationMessages) => ({
        ...prevConversationMessages,
        [message.receiverId]: [
          ...(prevConversationMessages[message.receiverId] || []),
          message,
        ],
      }));
    });
  ...
  const handleSendMessage = () => {
    ...
    setConversationMessages((prevConversationMessages) => ({
      ...prevConversationMessages,
      [receiverUser]: [
        ...(prevConversationMessages[receiverUser] || []),
        newMessageData,
      ],
    }));
    socket.emit("chatMessage", newMessageData);
    ...
    <div className="flex-1 bg-green-50 p-4 flex flex-col height-full">
                ...
                <div
                  className={`${
                    msg.senderId === loggedInUserId
                      ...
                      : "bg-gray-500"
                  } text-white font-semibold p-3 rounded-lg max-w-[70%] my-1`}
                >
                  {msg.message}
                </div>
              </div>
      ...
      <div className="flex items-center broder border-purple-500">
        <input
          ...
          className="flex-1 border rounded-lg p-2 mr-2 focus:border-green-500 outline-none"
        />
        <button
