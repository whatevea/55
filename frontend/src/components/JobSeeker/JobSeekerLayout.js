...
//         `/freelancer/getFilteredJobs?search=${text}`
//       );
...
//   return (
//     <div className="mb-10 border-2 border-red-600">
...
const JobSeekerLayout = () => {
  const [searchText, setSearchText] = useState(null);
  const [filteredJobs, setFilteredJobs] = useState([]);
  const [productFilteredJobs, setProductFilteredJobs] = useState([]);
  const [serviceFilteredJobs, setServiceFilteredJobs] = useState([]);
  ...
  const searchTextFunction = async (e) => {
    ...
    try {
      // Make a request to the backend endpoint to filter jobs
      const response = await http.get(
        `/freelancer/getFilteredJobs?search=${text}/${category}`
      );
      if (category === "Product") {
        setProductFilteredJobs(response.data);
      } else {
        setServiceFilteredJobs(response.data);
      }
      // setFilteredJobs(response.data); // Assuming the backend returns an array of filtered jobs
    } catch (error) {
      console.error("Error fetching filtered jobs:", error);
