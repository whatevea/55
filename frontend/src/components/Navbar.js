                    ...
                    <nav className="flex items-center justify-start md:justify-between h-16">
                        <div className="flex items-center justify-between">
                            <div className='lg:hidden w-[20px]'>
                                {isOpen ? (
                                    ...
                                    <i className="fa-solid fa-xmark fa-lg cursor-pointer" onClick={toggleMenu} />
                                ) : (
                                    // <FaBars className="fa-lg mr-8 cursor-pointer" onClick={toggleMenu} />
                                    <i className="fa-solid fa-bars fa-lg cursor-pointer" onClick={toggleMenu} />
                                )}
                            </div>
                            <Link to="/auth/register" className="md:ml-1"><svg className="w-[150px] h-[30px]" xmlns="http://www.w3.org/2000/svg"
                                ...
                                <ul className="flex justify-between gap-4 ml-4">
                                    {menuItems.map((item, index) => (
                                        <Link
                                            to={item.to}
                                            key={index}
                                            className={`parent-container ${hoveredIndex === index ? 'hovered' : ''} flex items-center hover:text-green-500 cursor-pointer text-sm`}
                                            onMouseEnter={() => setHoveredIndex(index)}
                                            onMouseLeave={() => setHoveredIndex(null)}
                                        >
                                            {item.label}
                                            <i className={`fa-solid fa-angle-down ml-1 mt-1 scale-80 ${hoveredIndex === index ? 'origin-center rotate-180 duration-300 ease-in-out' : ''}`}></i>
                                        </Link>
                                    ))}
                                </ul>
                            </div>
                        ...
                        <div className="justify-between gap-4  hidden md:flex">
                            <div className="lg:border  border-2 border-solid border-gray-300 rounded-xl flex items-center">
                                ...
                                <input className="outline-none bg-transparent p-1.5 hidden lg:flex" type="text" placeholder="Search " />
                            </div>
                            <div className="flex justify-between gap-4">
                                <Link className='hover:text-green-500 flex items-center' to="/auth/login">Login</Link>
                                <Link to="/auth/register" className="text-white px-3 py-1 hover:bg-green-500 rounded-md bg-green-600 flex items-center">Register</Link>
                            </div>
                        </div>
                    </nav>
                </header>

                {/* Conditionally render FindTalent component */}
                {hoveredIndex === 0 && (
                    <div className="absolute top-[57px] w-[100%] max-h-screen left-0 mt-2 z-20">
                        <FindTalent />
                    </div>
                )}
